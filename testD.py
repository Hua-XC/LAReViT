...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            inputO = Variable(input.cuda())
            inputA = Variable(fliplr(input).cuda())
            feat_pool, feat_fc = net_vit(inputO)
            feat_poolA, feat_fcA = net_vit(inputA)
            feat_pool=feat_pool+feat_poolA
            feat_fc=feat_fc+feat_fcA
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        ...
        distmat_pool = np.matmul(query_feat_fc, np.transpose(gall_feat_fc))
    ...
    for trial in range(10):
        test_trial = trial +1
        #model_path = checkpoint_path +  args.resume
        #regdb_HXC_p4_n6_lr_0.1_seed_0_SGD_trial_1_best
        model_path = checkpoint_path + 'regdb_HXC_p4_n6_lr_0.1_seed_0_SGD_trial_{}_best.t'.format(test_trial)
        ...
        if os.path.isfile(model_path):
            ...
            print('==> no_loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net_vit.load_state_dict(checkpoint['net'])
