...
def plot_embedding_2d(X,X_0, y, title=None):
    """Plot an embedding X with the class label y colored by the domain d."""
    XX=np.concatenate((X,X_0),axis=0)
    X_tsne = TSNE(n_components=2,random_state=33,perplexity=15,learning_rate=100).fit_transform(XX)
    X_tsne1 = X_tsne[0:1000]
    X_tsne2 = X_tsne[1000:2000]
    ...
    plt.scatter(X_tsne1[:, 0], X_tsne1[:, 1],s=10 ,c=col,cmap='tab20_r',label="Visible",marker='4',linewidth=0.5)
    plt.scatter(X_tsne2[:, 0], X_tsne2[:, 1],s=10 ,c=col,cmap='tab20_r',label="Infrared",marker='+',linewidth=0.5)
    plt.legend()
    # plt.subplot(122)
    ...
    plt.savefig('images/digits_tsne-pcaBBVITBASEAA.png', dpi=1000,format='png')
    plt.show()
